...
2. 尽量只生成 1 个产物，最多不超过 2 个。只有存在多个同样经典、确切的隐喻时，才生成第 2 个。绝不生成 3 个。
3. 优先选择直觉上最自然、最直接的产物。避免生成过于高级或偏僻的产物（如魔法物质、宇宙物质等），除非隐喻极其直接且无可替代。
4. 如果产物是元素列表中已有的元素，必须直接引用其现有 ID，并沿用其原 SVG 图标，绝不重复创建。绝对不要为同一个元素编写第二份不同的 SVG。
5. 如果产物是全新的，请调用 craft_elements 创建它：
...
6. 必须调用 craft_recipe 绑定本次的输入和输出。
