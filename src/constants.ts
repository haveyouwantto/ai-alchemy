...
2. 产物数量以 1 个为主（默认不生成副产物）。仅在确实存在多个同样经典、与同一反应紧密相关的隐喻时，才附加副产物；上限 3 个，且绝大多数合成应保持 1 个产物。产出列表中第一个元素是主产物（对应最核心、最经典的隐喻），其余是副产物。副产物必须是主产物的自然伴生（同一反应的不同面向、伴生物等），彼此独立不冲突，不得互相矛盾、不得重复、不得互为派生；副产物要克制，切勿让多个产出成为常态。
