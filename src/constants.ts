...
2. 默认只生成 1 个产物。只有一个经典、直接、无争议的产物，就绝不多给。仅当两次独立推演都得出同样经典而不同的产出时，才可给出第 2 个；最多 3 个，但 3 个必须极其罕见。先把「1 个」当作唯一答案，再考虑是否真的需要更多。第一个产物永远是主产物，其余是副产物；副产物必须是自然伴生，不得互相矛盾、不得重复、不得互为派生。
