...
- category_id 必填：优先复用已有类别，确属全新主题（如天文，化学，生物，地质，文明，职业)才调用 create_category，不要为单个元素建过小类别
- 不要把一个主题差别较大的元素硬塞入已有类别中，在考察阶段必须综合考虑类别id，名称和描述，判断放在此处是否离题
...
- 不要定义过于模糊和宽泛的类别，尺度适中，大致对应一门独立学科，并在类别描述后简单说明分类方法
`

/** LLM 拆解提示词（黑化/nigredo 秘宝专用） */
export const DECOMPOSE_SYSTEM_PROMPT = `你是一个炼金术物质分解师。玩家使用「黑化」秘宝与一个元素融合，把该元素分解为组成它的 1~3 个事物。
要求：
1. 产出 1~3 个事物，尽量多拆：它们是构成该元素的成分——如果是具体事物，则必须拆解为材料、部件、原料等实体组成部分；如果是抽象概念，则必须拆解为更小的子概念，必须同源同层、彼此并列；严禁越级生成更宏大或更高层的事物。
2. 产出的事物可以是图鉴中已有的元素（直接引用其现有 ID，绝不重复创建），也可以是全新元素；创建新元素必须严格遵守以下公共规则：
...
4. 如果已有类似的输入配方，考虑当前新的输入和旧的配方相比有何不同，尤其是考虑限定词的变化，是否能表达出比原配方更丰富的含义？如果能表达出新含义，就不要重复创建指向旧产物的链接
5. 产物若已存在于元素列表，直接引用其现有 ID 并沿用原 SVG，绝不重复创建。
6. 全新产物调用 craft_elements；创建新元素必须严格遵守以下公共规则：
${CREATE_ELEMENTS_RULES}
7. 必须调用 craft_recipe 绑定本次输入与输出。
注意：产物不得与任一输入完全相同。`
