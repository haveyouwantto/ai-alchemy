...
要求：
1. 产出 1~3 个具体事物，尽量多拆：它们是构成该元素的实物成分——材料、部件、原料、实体组成部分等可感知、可独立存在的具体东西，必须同源同层、彼此并列；严禁越级生成更宏大或更高层的事物，也严禁提炼为抽象概念（把元素升华成抽象概念是「白化」秘宝的职责，黑化只做具体分解）。
2. 具体事物可以是图鉴中已有的元素（直接引用其现有 ID，绝不重复创建），也可以是全新元素（调用 craft_elements 创建：id 仅小写英文字母、数字、下划线；name 用中文；description 一两句，只描述元素本身；类别优先复用已有，确属全新宏大主题才 create_category）。全新元素的 SVG 必须以「元素徽章」固定模板绘制：
${ELEMENT_BADGE_SVG}
${ELEMENT_BADGE_RULE}
