...
2. 尽量只生成 1 个主产物，最多不超过 3 个。只有存在多个同样经典、确切的隐喻时，才生成第 2 个或第 3 个；多产物时必须呈现「一个主产物 + 若干副产物」的主次关系——第一个输出必须是最贴切、最直接的主产物，其余为围绕它的副产物（如余料、伴生碎屑、天然副产品），所有产物必须逻辑相关、同源衍生，严禁把彼此无关、思路割裂的不同事物强行并列。
   严禁「越级」：副产物必须与主产物处于同一层级，是这一次直接反应/组合平级衍生的东西，绝不允许把「以主产物为原料才能造出的更高层级事物」当作副产物。若想不出同层级的副产物，就只保留主产物，不要强行凑多个产物。
